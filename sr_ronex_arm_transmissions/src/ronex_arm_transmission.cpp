...
/**
 * @file   robot_arm_transmission.hpp
...
#include <sr_ronex_arm_transmissions/ronex_arm_transmission.hpp>
...
PLUGINLIB_EXPORT_CLASS( ronex::RonexArmTransmission, pr2_mechanism_model::Transmission)

namespace ronex
{
  bool RonexArmTransmission::initXml(TiXmlElement *elt, pr2_mechanism_model::Robot *robot)
  {
    ...
    if (!joint)
    {
      ROS_ERROR("RonexArmTransmission could not find joint named \"%s\"", joint_name);
      return false;
    }
  ...
  bool RonexArmTransmission::initXml(TiXmlElement *elt)
  {
  ...
  void RonexArmTransmission::propagatePosition(std::vector<pr2_hardware_interface::Actuator*>& as,
  ...
  void RonexArmTransmission::propagatePositionBackwards(std::vector<pr2_mechanism_model::JointState*>& js,
                                                     std::vector<pr2_hardware_interface::Actuator*>& as)
  {
    //not doing anything (used in simulation)
  }

  void RonexArmTransmission::propagateEffort(std::vector<pr2_mechanism_model::JointState*>& js,
                                          std::vector<pr2_hardware_interface::Actuator*>& as)
  {
    for(ronex_iter_ = ronex_mappings_.begin(); ronex_iter_ != ronex_mappings_.end(); ++ronex_iter_)
    {
  ...
  void RonexArmTransmission::propagateEffortBackwards(std::vector<pr2_hardware_interface::Actuator*>& js,
